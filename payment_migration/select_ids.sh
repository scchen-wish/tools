...
if [ "$#" -lt 7 ]; then
  echo "Usage: $0 <username> <password> <host> <dbname> <table_name> <num_partitions> <selected_id_num>"
  exit 1
fi
...
#echo "min $min_id, max $max_id, range: $range"
...
for (( i=1; i<=$((NUM_PARTITIONS)); i++ )); do
  ...
  count_query="SELECT hex(id) FROM ${TABLE_NAME} WHERE id >= 0x$start_hex_uuid  AND id < 0x$end_hex_uuid limit ${ID_NUM};"

  # Execute the count query
  results=$(mysql -sN -B -e "${count_query}" -u "${DB_USER}" -p"${DB_PASSWORD}" -h "${DB_HOST}" "${DB_NAME}" 2>/dev/null)
